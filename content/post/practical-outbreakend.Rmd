...
tags: ["epicurve", "reproduction number"]
date: 2018-03-22
# Image by doodleguy under a Creative Commons Zero 1.0 Public Domain License  license -- https://openclipart.org/detail/261545/the-end-is-near
...
deps <- c("readxl", "tidyverse","pbapply")
...
- [`openxlsx`](https://cran.r-project.org/web/packages/openxlsx/index.html) to read `.xlsx` files without `rJava` dependence
- [`pbapply`](http://tidyverse.tidyverse.org) Package to get a progress bar for lengthy computations done using `apply`, `sapply`, and  `lapply`.
...
```
We skip the first 3 rows as the contain irrelevant header data. After loading the data sobe additional data munging is needed, which include converting the dates to the `Date` class and filling the missing values in the
...
## we substituted it with the date of laboratory confirmation.
linelist <- linelist %>%
...
The above data is the WHO data set on the
...
Using the RECON [`incidence`](http://www.repidemicsconsortium.org/incidence/) package its easy to plot the epicurve based on the date of symptoms onset in the linelist.
...
Focus of this blog entry is on the later part. Details on the first part is available in the R code from github.

### Parameter Estimation of the Serial Interval Distribution
...
The first step is accomplished in 
@nishiura_etal2016 by solving for given mean and standard deviation for the serial interval distribution observed in secondary data - see the [technical appendix](https://wwwnc.cdc.gov/eid/article/22/1/15-1383-techapp1.pdf) of the paper for details. The solution for $\alpha$ and $\beta$ can then be found analytically from these values.
```{r}
...
##
## @param t
## @param R_0         Estimated value of R_0
## @param k           Estimated value of k
## @param theta_serial
## @param oMax        Maximum value of o to try in the summation (instead of infinity)
##########################################################################
...
df <- df %>% 
  mutate(pi =  p_oneormore(t,R_0=R_0_hat, k=k_hat, theta_serial=theta_serial, yMax=250,verbose=FALSE))
## Look at the result
head(df, n=3)
```
...
sims <- pbapply(R0k_samples, 1, function(hat) {
  p_oneormore(df$t,R_0=hat[1], k=hat[2], theta_serial=theta_serial, yMax=250,verbose=FALSE)
})
...
- Michael Höhle, Stockholm University: RECON modified version of the blog post [No Sleep During the Reproducibility Session](http://staff.math.su.se/hoehle/blog/2016/08/04/outbreakEnd.html)
