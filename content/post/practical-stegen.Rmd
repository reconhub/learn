...
options seen before can be used for further customisation.
...
[dedicated website](https://www.repidemicsconsortium.org/incidence/). Here, we illustrate how incidence can be stratified e.g. gender:

```{r incidence_stratified}
i_sex <- incidence(stegen$date_onset, group = stegen$sex)
i_sex
as.data.frame(i_sex)
plot(i_sex)
```
...
plot(i_sex, border = "white") + scale_fill_manual(values = c(male = "#4775d1", female = "#cc6699")) +
