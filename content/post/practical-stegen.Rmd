...
2. variable types are partly wrong: 
    ...
    - *sex* should be a `factor` (i.e. a categorical variable)
...
of onset to actual `Date` objects, and sex to a categorical variable (`factor`):
```{r stegen_unique_ids}
...
And we use the function `recode` from the `dplyr' package to recode `sex` more
explicitely:
```{r stegen_recode}
...
We now have a cleaner dataset:

```{r stegen_cleaner_data}
stegen
```

We can save it for further records, as a `csv` file:
```{r stegen_save_data, eval = FALSE}
out_path <- here("data", "stegen_clean.csv")
write.csv(stegen, file = out_path)
```
...
## Summaries of ge and sex distribution
...
<p>**Going further** click on the button below to learn about `tapply`:</p>
  <button type="button" class="btn btn-info" data-toggle="collapse" data-target="#stegen_help_2">Tell me more</button>
  <div id="stegen_help_2" class="collapse">

`tapply` is a very handy function to stratify any kind of analyses. You will
find more details by reading the documentation of the functiton `?tapply`, but
...
<p>**Going further** click on the button below to learn about customising `ggplot2` graphics:</p>
  <button type="button" class="btn btn-info" data-toggle="collapse" data-target="#stegen_help_3">Tell me more</button>
...
```{r stegen-8, eval = FALSE}
# The first line below is read as follows:  assign a value of NA to stegen$pork WHERE stegen$pork is equal to 9
