...
categories: ["R"]
tags: ["Rmarkdown", "practical", "phylogenetics", "genetics"]
date: 2017-11-01T14:00:00-00:00
image: img/portfolio/logs1.jpg
showonlyimage: true
bibliography: biblioTJ.bib
---
...
This practical
requires a working version of \Rlogo~ \cite{np145} greater than or equal to
2.15.2.  It uses the following packages: *stats* implements basic
hierarchical clustering routines, *ade4* \cite{tj548} and
*adegenet* \cite{tjart05} are here used essentially for their graphics,
*ape* \cite{tj527} is the core package for phylogenetics, and
*phangorn* \cite{tj843} implements parsimony and likelihood-based
methods.  Make sure that the dependencies are installed as well when installing
the packages:
...
implemented in *adegenet* \cite{tjart23}.
...
details).  Here, we use Tamura and Nei 1993's model \cite{tj841} which allows
...
practice, simple yet flexible models such as that of Tamura and Nei (1993,
\cite{tj841}) are probably fair choices.  At the very least, the genetic
...
refer to this model as $\mbox{GTR} + \Gamma(4)$ (GTR: global time reversible).
...
Nei's distance \cite{tj841}.  However, we can note that it is remarkably similar
