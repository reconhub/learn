...
- *Funciones básicas o primitivas*: estas son las funciones predeterminadas en _R_ bajo el _ base package (paquete base)_. Por ejemplo, pueden incluir operaciones aritméticas básicas, pero también operaciones más complejas como la extracción de valores medianos `median (mediana)` o `summary (resumen)` de una variable.
...
Por ejemplo, para una distribución normal se usa `?dnorm` para explorar los argumentos en esta función

 - `dnorm` = función de densidad con argumentos predeterminados `(x, mean = 0, sd = 1, log = FALSE)`
...
Guarde este conjunto de datos en el mismo directorio en el que se esta trabajando actualmente.
...
Del paquete `dyplr`, las funciones más comunes son:
 ...
 - `arrange`: organiza el conjunto de datos por el valor de una variable particular si es numérico, o por orden alfabético si es un carácter.
...
Una cosa importante a tener en cuenta es que para una sola instrucción, ggplot solo usará variables que pertenezcan al mismo conjunto de datos. Entonces, necesitamos tener las tres variables (x, y y colour) en el mismo marco de datos (con la misma longitud).
