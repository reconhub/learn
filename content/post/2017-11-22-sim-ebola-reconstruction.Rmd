...
gt_params
gt <- distcrete("gamma", shape = gt_params$shape, scale = gt_params$scale, interval = 1L, w = 0)
gt
...
has been reported to have a mean of 9.1 with a standard deviation of 7.3. Call
this object `incub`, and plot the distribution, which should look like this:
...
incub_params
incub <- distcrete("gamma", shape = incub_params$shape, scale = incub_params$scale, interval = 1L, w = 0)
incub
...
data <- outbreaker_data(dates = linelist$onset, ctd = ctd, w_dens = gt$d(1:100))
