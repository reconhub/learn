...
- [`xlsx`](https://cran.r-project.org/web/packages/xlsx/index.html) to read `.xlsx` files
...
previous data on the serial interval, create this distribution using the
function `distcrete` from the similarly named package. Specify make sure you use
daily intervals for the discretisation, and use `w = 0` to indicate dates should
be floored to the day. Store the ouput in an object called `si`. Your results
should look like:

```{r si}
library(distcrete)
si_params <- gamma_mucv2shapescale(si_fit$mu, si_fit$cv)
si <- distcrete("gamma",
...
library(earlyR)
mu <- si_fit$mu # mean in days days
sigma <- si_fit$sd # standard deviation in days
