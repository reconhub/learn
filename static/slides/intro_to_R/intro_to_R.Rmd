...
- code is not compiled, but \textit{interpreted}\pause
- no file, all in the RAM\pause
- content of a session can be saved using `save`/`save.image` (output: `.RData` file)\pause
- `.RData` files can be loaded using `load`\pause
- data, results, functions, etc. are all \textit{objects}\pause
...
## Type of objects 3: categorical variables
...
A `data.frame` is a table where different variables (columns) can have different
types:
...
A `list` can store any collection of objects of any types in different `slots`:
...
- `integer`: integer numbers\pause
- `numeric`: decimal numbers\pause
- `character`: character strings\pause
- `factor`: categorical variables\pause
- `vector`: collection of values (same type)\pause
- `matrix`: collection of values (table, same type)\pause
- `data.frame`: table where columns can have different types, but same length\pause
- `list`: collection of elements, no restriction of content\pause
- `...`: classes for: DNA sequences, graphs, phylogenetic trees, etc.
